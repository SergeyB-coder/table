<template>
    ...
    <modal-filters-reports v-show="show_report_filters" :show_segments_list_id="show_segments_list_id" @toggle-segments-list="toggleSegmentsList"/>
        ...
        <div class="cntr-bg-2">
            ...
            <div class="text-1">{{
                settings_mode ? 'Настройки' :
                    addexport_mode ? 'Добавить экспорт' : 'Мониторинг трафика доставок' }}</div>
            ...
            <div class="segment_menu mt-18 pos-rel">
                ...
                <div class="report_menu" v-show="is_settings_mode || report_mode || addexport_mode">
                    ...
                    <div @click="() => { new_user ? new_user = false : addexport_mode = false; config_mode = false; is_settings_mode = false; report_mode = false }"
                        v-show="report_mode" class="grey-box cntr-row a-c menu_filters pos-rel cur-pointer h-40">
                        ...
                        <div @click="() => { show_modal = !show_modal; is_settings_mode = false; report_mode = false }"
                    ...
                    <div @click="() => { new_user ? new_user = false : config_mode = false }"
                        ...
                        <div @click="() => { show_report_filters = !show_report_filters; }"
                            class=" text-2 ml-11 cur-pointer">Фильтры</div>
                    </div>
                </div>
                ...
                <div class="segment_menu" v-show="!report_mode && !settings_mode && !addexport_mode">
                    ...
                    <div class="grey-box cntr-row a-c menu_filters pos-rel cur-pointer h-40"
                        @click="error_export = !is_check; show_export_success = !show_export_success" v-show="!config_mode">
                        ...
                        <div class=" text-2 ml-11 cur-pointer">Выгрузить</div>
                    </div>
                    <!-- configure bnt -->
            ...
            <div class="cntr-table" :class="{ h_75: new_user }">
                ...
                <table class="scroll_table" v-show="!new_user && !report_mode && !addexport_mode">
                    ...
                    <tr v-for="segment in segments" v-bind:key="segment.id">
                        ...
                        <td>
                            <div class="td__segment_settings">
                                <div v-for="setting in segment.settings" :key="setting.title">
                                    <strong>{{ setting.title }}:</strong> {{ setting.text }}
                                </div>
                            </div>
                        </td>
                        <td>
                            <!-- <label class="ml-8 active_color_link">{{ segment.status }}</label> -->
                            <svg v-show="segment.status === 'check'" width="20" height="20" viewBox="0 0 20 20"
                            ...
                            <svg v-show="segment.status === 'lightning'" width="20" height="20" viewBox="0 0 20 20"
                                fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path
                            ...
                            <icon-tooltip v-show="segment.status === 'caution'" iconType="caution"
                                tooltipText="Ошибка" />
                            <icon-tooltip v-show="segment.status === 'greenlightning'" iconType="greenlightning"
                                tooltipText="Создана" />
                            <icon-tooltip v-show="segment.status === 'greenpause'" iconType="greenpause"
                                tooltipText="Выполняется" />
                            <icon-tooltip v-show="segment.status === 'redpause'" iconType="redpause"
                                tooltipText="Выполняется" />
                            <icon-tooltip v-show="segment.status === 'redplay'" iconType="redplay"
                                tooltipText="Остановлена" />

                        </td>
                        <td>
                            <label>{{ segment.date_create }}</label>
                        </td>
                        <td>
                            <!-- <svg-cancelred v-if="segment.status === 'check'" />
                            <svg-check v-else /> -->
                ...
                <div v-show="report_mode" style="overflow-x: scroll; height: 100%;">
                ...
                <div v-show="addexport_mode" style="width: 100%">
                    <div class="cntr-row" style="gap: 20px">
                        <div style="width: 50%">
                            <div class="cntr-row">
                                <input placeholder="Доставки"
                                    class="input_add_export_segments txt_input_delivers mon_seg_add_exp_part1" />
                                <div @click="show_modal_list_delivers = true"
                                    class="input_add_export txt_input mon_seg_add_exp_part2">
                                    Списком
                                </div>
                            </div>
                            <div class="p-r" style="margin-top: 20px;">
                                <input :class="{ wrong_data: is_wrong_data }"
                                    class="input_add_export_segments txt_input" placeholder="Все сети" />
                                <svg @click="show_selecter_net = !show_selecter_net"
                                    @mouseleave="input_bg_active = false" @mouseenter="input_bg_active = true"
                                    class="input__svg_arrow" :class="{ rotate_180: show_selecter_net }" width="16"
                                    height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd" clip-rule="evenodd"
                                ...
                                <!-- <selecter-role v-show="show_select_item" :roles="['Администратор', 'Пользователь']" /> -->
                                <selecter-net v-show="show_selecter_net" :cntr_width="my_width" />
                            </div>
                        </div>
                        <div style="width: 50%">
                            <div class="p-r">
                                <input placeholder="Все форматы" class="input_add_export_segments txt_input" />
                                <svg @click="show_select_item = !show_select_item" @mouseleave="input_bg_active = false"
                                    ...
                                    :class="{ rotate_180: show_select_item }" width="16" height="16" viewBox="0 0 16 16"
                                    fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd" clip-rule="evenodd"
                                        d="M1.64592 4.64592C1.69236 4.59935 1.74754 4.56241 1.80828 4.5372C1.86903 4.512 1.93415 4.49902 1.99992 4.49902C2.06568 4.49902 2.13081 4.512 2.19155 4.5372C2.2523 4.56241 2.30747 4.59935 2.35392 4.64592L7.99992 10.2929L13.6459 4.64592C13.6924 4.59943 13.7476 4.56255 13.8083 4.53739C13.8691 4.51223 13.9342 4.49929 13.9999 4.49929C14.0657 4.49929 14.1308 4.51223 14.1915 4.53739C14.2522 4.56255 14.3074 4.59943 14.3539 4.64592C14.4004 4.6924 14.4373 4.74759 14.4624 4.80833C14.4876 4.86907 14.5005 4.93417 14.5005 4.99992C14.5005 5.06566 14.4876 5.13076 14.4624 5.1915C14.4373 5.25224 14.4004 5.30743 14.3539 5.35392L8.35392 11.3539C8.30747 11.4005 8.2523 11.4374 8.19155 11.4626C8.13081 11.4878 8.06568 11.5008 7.99992 11.5008C7.93415 11.5008 7.86903 11.4878 7.80828 11.4626C7.74754 11.4374 7.69236 11.4005 7.64592 11.3539L1.64592 5.35392C1.59935 5.30747 1.56241 5.2523 1.5372 5.19155C1.512 5.13081 1.49902 5.06568 1.49902 4.99992C1.49902 4.93415 1.512 4.86903 1.5372 4.80828C1.56241 4.74754 1.59935 4.69236 1.64592 4.64592Z"
                                        fill="#6B7280" />
                                ...
                                <selecter-role v-show="show_select_item"
                                    :roles="['Все форматы', 'Формат1', 'Формат2']" />
                            </div>

                            <div class="p-r" style="margin-top: 20px;">
                                <input :class="{ wrong_data: is_wrong_data }"
                                    class="input_add_export_segments txt_input" placeholder="10.10.2000" />
                                <svg-data
            ...
            segments: [
                {
                    id: 1,
                    settings: [
                        { title: 'Пользователь', text: 'Логин пользователя' },
                        { title: 'Доставки', text: 'перечисление1, переисление2, перечисление3, перечисление4' },
                        { title: 'Сети', text: 'Все' },
                    ],
                    status: 'check',
